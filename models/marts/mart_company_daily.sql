...
with
    ...
    company as (select * from {{ ref("dim_companies") }}),

    subscriptions as (select * from {{ ref("fct_daily_subscription_revenue") }}),

    platform_usage as (select * from {{ ref("fct_daily_platform_usage") }}),

    support_ticket_signals as (select * from {{ ref("fct_daily_support_tickets") }}),

    invoices as (select * from {{ ref("fct_monthly_invoice_status") }}),
    ...
    final as (
        ...
        select
            ...
            case
                when coalesce(s.active_mrr_sek, 0) = 0
                then 'inactive'
                when coalesce(i.overdue_invoices, 0) > 0
                then 'billing_risk'
                when coalesce(sts.high_priority_tickets, 0) > 2
                then 'high_risk'
                when coalesce(pu.total_events, 0) = 0
                then 'usage_risk'
                else 'healthy'
        ...
        left join
            platform_usage as pu
            on cd.date_day = pu.event_date
            and cd.company_id = pu.company_id

        left join
            support_ticket_signals as sts
            on cd.date_day = sts.date_day
            and cd.company_id = sts.company_id

        left join
            invoices as i
            on cd.company_id = i.company_id
